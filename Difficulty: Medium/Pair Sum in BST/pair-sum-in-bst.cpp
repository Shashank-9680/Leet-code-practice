...
class Solution {
  ...
  bool solve(Node* root, int target,Node* mainroot){
        if(!root)return false;
        int find=target-(root->data);
        if(find!=root->data && check(mainroot,find))return true;
        return solve(root->left,target,mainroot) or solve(root->right,target,mainroot);
